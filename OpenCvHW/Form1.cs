    ...
    public partial class Form1 : Form
    {
        ...
        async void objectTracking()
        {
            ...
            while (FrameNo <= TotalFrames)
            {
                ...
                capture.Read(frame);
                CvInvoke.MorphologyEx(frame, Enhancedframe, MorphOp.Close, se, new Point(-1, -1), 1, BorderType.Default, CvInvoke.MorphologyDefaultBorderValue);
                ImgBlurred.Image = frame;

                CvInvoke.CvtColor(Enhancedframe, HSVFrame, ColorConversion.Bgr2Hsv);
                CvInvoke.InRange(HSVFrame, new ScalarArray(new MCvScalar(100, 200, 175)), new ScalarArray(new MCvScalar(255, 255, 255)), binaryImg);
                ...
                CvFunctions.DrawContours(EnhancedBinaryImg, frame, green, red);
                imgContour.Image = frame;
